...
public class TextRenderer {
    ...
    public TextRenderer()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, 1000000, GL_DYNAMIC_DRAW);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, 1000000, GL_DYNAMIC_DRAW);
        glBindVertexArray(0);
    }

    private void setupRenderData(List<GUILabel> guiLabels)
    {
        //TODO: Optimize updating of lists.
        vertexData.clear();
        indices.clear();
        matrices.clear();
        for (GUILabel guiLabel : guiLabels)
        {
            ...
            List<Float> matrixArray = ArrayUtils.toListF(guiLabel.getMatrixArray());
        ...
        glBindBuffer(GL_ARRAY_BUFFER, textVBO);
        glBufferSubData(GL_ARRAY_BUFFER, 0, ArrayUtils.toPrimitiveArrayF(vertexData));
        glBindBuffer(GL_ARRAY_BUFFER, textMatrixBuffer);
        glBufferSubData(GL_ARRAY_BUFFER, 0, ArrayUtils.toPrimitiveArrayF(matrices));
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, textEBO);
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, ArrayUtils.toPrimitiveArrayI(indices));

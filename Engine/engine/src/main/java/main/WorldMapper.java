            ...
            for (int y = 0; y < grid.getHeight(); ++y)
            {
                for (Tile tile : grid.getGridCell(x, y).getTiles())
                {
                    if (!(tile instanceof TileTree))
                    {
                        ...
                        if (color != null)
                        {
                            worldMap.setRGB(Main.fastFloor(tile.getPosition().getX()), height - Main.fastFloor(tile.getPosition().getY()) - 1, color.getRGB());
                        }
                    }else
                    {
                        ...
                        for (int z = 0; z > -3; --z)
                        {
                            worldMap.setRGB(Main.fastFloor(tile.getPosition().getX()), height - Main.fastFloor(tile.getPosition().getY()) - 1 - z, color.getRGB());
                        }
                    }
                }
            }

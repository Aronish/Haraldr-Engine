...
public class MaterialParser
{
    ...
    public static JsonMaterial parseMaterial(JSONObject materialDefinition) throws JSONException
    {
        ...
        JSONObject variant;
        if (specification.getJSONObject("materials").getJSONObject(type).has("variants"))
        {
            variant = getBestVariantCandidate(materialProperties, specification.getJSONObject("materials").getJSONObject(type).getJSONObject("variants"));
        } else
        {
    ...
    private static @NotNull List<ShaderSampler> extractSamplers(JSONObject materialProperties, JSONObject samplerDefinition, List<String> optionalSamplerSwitches)
    {
        ...
        for (String samplerToken : samplerDefinition.keySet())
        {
            ...
            {
                if (!materialProperties.has(samplerToken)) throw new IllegalStateException("Missing material property : " + samplerToken);
